...
public class Puzzle1 {
    ...
    public static boolean isValid(String line) {
        ...
        int min = Integer.valueOf(counts[0]);
        int max = Integer.valueOf(counts[1]);
        char c = items[1].charAt(0);
        String password = items[2];

        if (TRACE) System.out.println(
                String.format("Looking for %d to %d occurrences of '%c' in '%s'.", min, max, c, password)
        );

        int o = occurrences(c, password);
